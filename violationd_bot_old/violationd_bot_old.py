...
# Persistent retry queue (failed accounts/groups)
FAILED_QUEUE_FILE = "failed_processing_queue.json"
        ...
        if config_path.exists():
            ...
            _config = {
                ...
                "api": {
                    ...
                    "vps_timeout": 930,
                    "token_retries": 3,
                    "token_retry_delay_seconds": 60,
                    "vps_retries": 3,
                    "vps_retry_delay_seconds": 120,
                    "queryinfo_retries": 3,
                    "queryinfo_retry_delay_seconds": 60
                },
                "email": {
                ...
                "processing": {
                    "sleep_between_accounts_seconds": 120
                },
                "exclusions": {
...
def load_failed_queue():
    """
    Load persistent failed-processing queue.
    Structure:
    {
      "version": 1,
      "updated_at": "...iso...",
    ...
    """
    path = Path(FAILED_QUEUE_FILE)
    if not path.exists():
        return {"version": 1, "updated_at": None, "failed_accounts": {}}
    try:
        ...
        data.setdefault("failed_accounts", {})
        if not isinstance(data["failed_accounts"], dict):
            data["failed_accounts"] = {}
        return data
    except Exception as e:
        logger.warning(f" Failed to load {FAILED_QUEUE_FILE}: {e} (starting fresh)")
        return {"version": 1, "updated_at": None, "failed_accounts": {}}


def save_failed_queue(queue_data):
    """Persist failed-processing queue to disk."""
    try:
        queue_data["updated_at"] = datetime.now().isoformat()
        with open(FAILED_QUEUE_FILE, "w", encoding="utf-8") as f:
            json.dump(queue_data, f, indent=2, ensure_ascii=False)
    except Exception as e:
        logger.error(f" Error saving {FAILED_QUEUE_FILE}: {e}")


def mark_account_failed(queue_data, email, username_group, error):
    """Add/update a failed account entry."""
    failed_accounts = queue_data.setdefault("failed_accounts", {})
    entry = failed_accounts.get(email, {})
...
def clear_account_failed(queue_data, email):
    """Remove an account from failed queue if it later succeeds."""
    failed_accounts = queue_data.get("failed_accounts", {})
    if isinstance(failed_accounts, dict) and email in failed_accounts:
        failed_accounts.pop(email, None)
...
def _retry(operation_name, attempts, delay_seconds, func):
    """
    Retry helper for network-ish calls.
    - attempts: total attempts (>=1)
    - delay_seconds: constant delay between attempts
    """
    last_exc = None
    ...
    for attempt in range(1, attempts + 1):
        try:
            if attempt > 1:
                logger.warning(f" Retry {attempt}/{attempts} for {operation_name} (sleep {delay_seconds}s)")
                if delay_seconds > 0:
                    time.sleep(delay_seconds)
            return func()
        except Exception as e:
            last_exc = e
            logger.warning(f" {operation_name} attempt {attempt}/{attempts} failed: {e}")
    # if func never succeeded, re-raise last exception so caller can mark failed with reason
    raise last_exc
...
def call_vps(email):
    ...
    vps_timeout = api_config.get("vps_timeout", 930)
...
def get_account_data(email, stored_tokens):
    ...
    config = get_config()
    api_config = config.get("api", {})
    token_retries = api_config.get("token_retries", 3)
    token_retry_delay_seconds = api_config.get("token_retry_delay_seconds", 60)
    vps_retries = api_config.get("vps_retries", 3)
    vps_retry_delay_seconds = api_config.get("vps_retry_delay_seconds", 120)
    queryinfo_retries = api_config.get("queryinfo_retries", 3)
    queryinfo_retry_delay_seconds = api_config.get("queryinfo_retry_delay_seconds", 60)

    if token:
        logger.info(f"✓ Token found for {email}")
        try:
            api_response = _retry(
                operation_name=f"call_with_token({email})",
                attempts=token_retries,
                delay_seconds=token_retry_delay_seconds,
                func=lambda: call_with_token(token),
            )
        except Exception as e:
            api_response = None
            last_error = f"call_with_token retry failed: {e}"
        if api_response:
            ...
            if api_response.get("successful") or api_response.get("errCode") == 0:
                ...
                data = None
                last_error = f"call_with_token returned unsuccessful: errCode={api_response.get('errCode')} message={api_response.get('message')}"
        else:
            logger.warning(" No data returned from API call")
            data = None
            last_error = "call_with_token returned None"
    ...
    if not data or not data.get("userInfo"):
        ...
        try:
            ...
            vps_resp = _retry(
                operation_name=f"call_vps({email})",
                attempts=vps_retries,
                delay_seconds=vps_retry_delay_seconds,
                func=lambda: call_vps(email),
            )
            ...
            if vps_resp.get("successful") and vps_resp.get("data", {}).get("token"):
                ...
                try:
                    ...
                    def _do_queryinfo():
                        ...
                        return resp.json()
                    user_info = _retry(
                        operation_name=f"queryInfo({email})",
                        attempts=queryinfo_retries,
                        delay_seconds=queryinfo_retry_delay_seconds,
                        func=_do_queryinfo,
                    )
                ...
                return None, f"VPS response missing token: {vps_resp}"
        except Exception as e:
            logger.error(f" VPS call failed: {e}")
            return None, f"VPS call failed: {e}"
    ...
    if not user_info:
        ...
        return None, (data.get("error") if isinstance(data, dict) and data.get("error") else (last_error or "No userInfo"))
...
def process_emails(email_list, failed_queue=None):
    ...
    for username, group_emails in username_groups.items():
        ...
        processing_cfg = get_config().get("processing", {})
        sleep_between_accounts_seconds = processing_cfg.get("sleep_between_accounts_seconds", 120)

        for idx, email in enumerate(group_emails):
            devices, err = get_account_data(email, stored_tokens)
            if devices is not None:
                group_accounts_data[email] = devices
                # If this account was previously failed, clear it now
                if failed_queue is not None:
                    clear_account_failed(failed_queue, email)
            else:
                ...
                if failed_queue is not None:
                    mark_account_failed(
                        ...
                        error=err or "get_account_data failed (returned None)",
                    )

            # Sleep between account processing (user-requested throttling)
            # Do not sleep after the last account in the group.
            if idx < len(group_emails) - 1:
                try:
                    delay = float(sleep_between_accounts_seconds or 0)
                except Exception:
                    delay = 120.0
                if delay > 0:
                    logger.info(f" Sleeping {delay}s before next account in group '{username}'...")
                    time.sleep(delay)
        
        if not group_accounts_data:
            logger.warning(f" No account data collected for group '{username}'. Skipping to next group.")
            # Persist failures immediately so they can be retried first next cycle
            if failed_queue is not None:
...
def run_continuous_monitoring():
    ...
    while True:
        try:
            ...
            # Load failed queue at the start of each cycle (so failures are retried first)
            failed_queue = load_failed_queue()
            failed_accounts = failed_queue.get("failed_accounts", {}) if isinstance(failed_queue, dict) else {}
            failed_emails = list(failed_accounts.keys()) if isinstance(failed_accounts, dict) else []
            if failed_emails:
                logger.warning(f" Found {len(failed_emails)} failed account(s) from previous runs. These will be processed FIRST this cycle.")
            ...
            # Step 1.5: Process failed accounts first (so they don't starve)
            if failed_emails:
                logger.info("")
                logger.info(f" Step 1.5: Processing FAILED accounts first ({len(failed_emails)} total)...")
                process_emails(failed_emails, failed_queue=failed_queue)
            ...
            if new_emails:
                ...
                process_emails(new_emails, failed_queue=failed_queue)
            ...
            process_emails(api_emails, failed_queue=failed_queue)
